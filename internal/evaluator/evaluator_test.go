...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{`{"name":"emad"}[fn(x){x}];`, "unusable as hash key: FUNCTION"},
	}
...
func TestLetStatement(t *testing.T) {
	tests := []struct {
		...
		{"let a = 5*5;let b = a;let c = a + b;c;", 50},
	}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong parameters. Parameters=%T(%+v)", evaluated, evaluated)
	}
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not Arrya. got=%T (%+v)",evaluated,evaluated)
	}

	if len(result.Elements) != 3 {
		t.Fatalf("array has wrong num of elements.expected=3 got=%d",len(result.Elements))
	}
